...
> Answer:
> Client-side routing does not require re-rendering a whole page between views, so while initial loading is slower to get all the data, routing between views is generally faster. Smoother transitions and no white flash between loads.

- [ ] What does AJAX stand for?

  > Answer:
  > Asynchronous JavaScript and XML - a technique for creating faster and more interactive web applications.

- [ ] What are `controlled components` in React?

  > Answer: A component that handles data - every mutation is handled by a handler function. State is provided by React rather than internally.
